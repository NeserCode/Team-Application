  ...
  <div class="userDetail">
    <div class="userInfo">
      <div class="settingOption userZone">
        <User center :isUserImageRound="true" />
        ...
        <span class="realname" v-show="!isUserRename"
          >用户名 | <span class="text">{{ settings.userInfo.name }}</span></span
        >
      </div>
      <span class="introduce selectable">{{
        settings.userInfo.introduce ?? "(这家伙啥也没有写下来)"
      }}</span>
      <div class="settingOption">
        <span class="preText"> 性别 </span>
        <el-tag>{{
          (settings.userInfo.sex == "m"
            ? "男"
            : settings.userInfo.sex == "w"
            ? "女"
            : null) ?? "Unknow"
        }}</el-tag>
      </div>
      ...
      <div class="settingOption">
        ...
        <el-tag>
          <span>{{ settings.userInfo.value ?? "User" }}</span></el-tag
        >
        <el-divider direction="vertical"></el-divider>
        <el-tag>
          <i class="el-icon-key"></i
          ><span class="selectable">{{
            settings.userInfo.key ?? "Warn: No_Such_Key"
          }}</span>
        </el-tag>
      </div>
      <div class="settingOption">
        ...
        <el-tag @click="handleUserBound">{{
          settings.userInfo.bound ?? "Email 未绑定"
        }}</el-tag>
      </div>
    </div>
    <el-button class="clearLoginBtn" type="danger" @click="isConfirmOut = true"
      >清除登录状态</el-button
    >
...
.settingOption {
  @apply h-16 w-2/3 mx-auto py-4 select-none;
  line-height: 2rem;
}
.preText {
